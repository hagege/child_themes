...
/**
 * Add body class for hide header functionality
 * Diese Funktion sollte in die Hauptdatei eingefügt werden
 */
function slsh_add_body_class($classes) {
    $hide_header = get_option('slsh_hide_header', 'no');
    if ('yes' === $hide_header) {
        $classes[] = 'slsh-hide-header-enabled';
    }
    return $classes;
}
add_filter('body_class', 'slsh_add_body_class');

/**
 * Load Dynamic CSS for Plugin Shrinking Logo Sticky Header
 * Diese Funktion ersetzt die bestehende slsh_dynamic_css() Funktion
 */
function slsh_dynamic_css(): void {
	...
	// CSS für Hide Header Funktionalität - nur wenn aktiviert
	if ( 'yes' === $hide_header ) {
		$custom_css .= "
			/* Hide Header Functionality */
			body.slsh-hide-header-enabled header.wp-block-template-part {
				transition: transform {$anim_duration}s cubic-bezier(.4,0,.2,1), height {$anim_duration}s cubic-bezier(.4,0,.2,1), background-color {$anim_duration}s;
			}
			body.slsh-hide-header-enabled header.wp-block-template-part.hide-header {
				transform: translateY(-100%);
			}
			body.slsh-hide-header-enabled header.wp-block-template-part.show-header {
	...
	if ( $nav_breakpoint > 599 ) {
		...
		/* Fallback auf Standard-Breakpoint */
		$nav_breakpoint = 599;
	}
	
	// Off-Canvas CSS - verbessert für bessere Theme-Kompatibilität
	if ( 'yes' === $enable_off_canvas ) {
		$custom_css .= "
		/* Enhanced Off-Canvas Styles */
		@media (max-width: {$nav_breakpoint}px) {
			.wp-block-navigation__responsive-container {
				...
				transition: right {$off_canvas_speed}s ease-in-out;
			...
			/* Overlay Styling */
			.wp-block-navigation__responsive-dialog-overlay {
				z-index: 1000 !important;
			}
			
			/* Stelle sicher, dass der Header über allem anderen liegt wenn Off-Canvas offen ist */
			body.slsh-hide-header-enabled .wp-block-navigation__responsive-container.is-menu-open ~ header.wp-block-template-part,
			body.slsh-hide-header-enabled header.wp-block-template-part:has(+ * .wp-block-navigation__responsive-container.is-menu-open) {
