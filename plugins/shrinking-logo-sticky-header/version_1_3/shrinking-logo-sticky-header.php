...
function slsh_register_settings(): void {
	...
	// Option for Submenue
	add_option( 'slsh_enable_submenu', 'no' );	
	...
	<div>
		...
		<form method="post" action="<?php echo esc_url( get_admin_url() ); ?>options.php">
			...
			<table>
				...
				<tr>
					<th scope="row"><h3 style="text-align: left;margin-top:30px">Expandable submenu</h3></th>
				</tr>
				<tr>
					<th scope="row"><label style="display: block; text-align: left" for="slsh_enable_submenu"><?php esc_html_e( 'Should it be possible to expand submenus?', 'shrinking-logo-sticky-header' ); ?></label></th>
					<td>
						<input type="checkbox" id="slsh_enable_submenu" name="slsh_enable_submenu" value="yes" <?php checked( 'yes', get_option( 'slsh_enable_submenu', 'no' ) ); ?> />
						<span><?php esc_html_e( 'Should it be possible to expand submenus?', 'shrinking-logo-sticky-header' ); ?></span>
					</td>
				</tr>
...
function slsh_dynamic_css(): void {
	...
	$enable_submenu     = get_option( 'slsh_enable_submenu', 'no' );
	...
	if ( 'yes' === $enable_submenu ) {
		$custom_css .= "
		/* Enable expandable Submenu */
...
add_action( 'wp_enqueue_scripts', 'slsh_js_enqueue' );

function slsh_submenu_enqueue(): void {
	wp_enqueue_script(
		'slsh_submenu_js',
		plugins_url( 'assets/js/slsh_submenu.js', __FILE__ ),
		array(),
		SLSH_VERSION,
		true
	);
}
add_action( 'wp_enqueue_scripts', 'slsh_submenu_enqueue' );
