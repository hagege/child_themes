...
Tested with Twenty Twenty-Three, Twenty Twenty-Four, Twenty Twenty-Five, Ollie, Circles WP. 
...
= The spacing above and below changes when you scroll down (shrink). How can this be changed? =
If you set the values for the top and bottom paddings to 0 in the settings for the header (and possibly for all groups contained in the header), 
the inner margins will be controlled by the plugin, and the margins (paddings) should be set correctly.
You can find the settings in the header template section (see Appearance -> Editor -> Patterns -> Header)
