...
###################################### Vereisten ########################################
#              Controle als alle vereiste functions beschikbaar zijn.                   #
#########################################################################################
if ! hash awk 2>/dev/null
then
    echo "'awk' ist nicht nutzbar."
    exit 1
elif ! hash cut 2>/dev/null
then
    echo "'cut' ist nicht nutzbar."
    exit 1
elif ! hash egrep 2>/dev/null
then
    echo "'egrep' ist nicht nutzbar."
    exit 1
elif ! hash find 2>/dev/null
then
    echo "'find' ist nicht nutzbar."
    exit 1
elif ! hash grep 2>/dev/null
then
    echo "'grep' ist nicht nutzbar."
    exit 1
elif ! hash printf 2>/dev/null
then
    echo "'printf' ist nicht nutzbar."
    exit 1
elif ! hash uniq 2>/dev/null
then
    echo "'uniq' ist nicht nutzbar."
    exit 1
elif ! hash xargs 2>/dev/null
then
    echo "'xargs' ist nicht nutzbar."
    exit 1
else
    #alles okay, scherm leegmaken
    clear
fi

###################################### Globals ##########################################
#     Instellen van de globale variabelen en functies die het script gebruikt.          #
#########################################################################################
...
###################################### Globals ##########################################
#             Declareren van globalen gebruikt in verschillende functies                #
#########################################################################################
...
################################ Malware definitions ####################################
#              Malware definities voor de detectie van malafide bestanden               #
#########################################################################################
...
function create_backup {
    echo "Wird in Version 1.1 hinzugefügt."
...
function sweep_wp_core {
    ...
    clear
    # Notice onscreen
    echo "WordPress $version erkannt. Ersetzen der Core-Dateien."
    echo "Warten Sie mal ..."
    ...
    echo "######### WordPress Core #########" >> $workingdirectory/wpsweeper-$datetime.log
    echo "" >> $workingdirectory/wpsweeper-$datetime.log
    echo "Ersetzen der WordPress-Version $version durch neue Kerndateien, die aus dem WordPress.org-Repository heruntergeladen wurden." >> $workingdirectory/wpsweeper-$datetime.log
    echo "" >> $workingdirectory/wpsweeper-$datetime.log
...
function plugin_start_logentry {
    echo "Ersetzen der Plugins durch eine neue Version, die aus dem WordPress.org-Repository heruntergeladen wurde."
    echo "######### WordPress Plugins #########" >> $workingdirectory/wpsweeper-$datetime.log
    echo "" >> $workingdirectory/wpsweeper-$datetime.log
    echo "Nachfolgend finden Sie die Plugins, die erfolgreich durch eine neue Version ersetzt wurden, die aus dem Plugin-Repository von WordPress.org heruntergeladen wurde." >> $workingdirectory/wpsweeper-$datetime.log
    echo "Wenn ein Plugin als FEHLER aufgeführt wird, hat dies einen der folgenden Gründe:" >> $workingdirectory/wpsweeper-$datetime.log
    echo "- Das Plugin ist ein Premium-Plugin (das wir nicht automatisch für Sie ersetzen können)." >> $workingdirectory/wpsweeper-$datetime.log
    echo "- Das Plugin wurde bereits aus dem Plugin-Repository von WordPress.org entfernt." >> $workingdirectory/wpsweeper-$datetime.log
    echo "- Das Plugin ist ein benutzerdefiniertes Plugin, das nicht im Plugin-Repository von WordPress.org veröffentlicht wird." >> $workingdirectory/wpsweeper-$datetime.log
    echo "" >> $workingdirectory/wpsweeper-$datetime.log
    echo ""
...
function sweep_plugin_noversion {
    ...
    if test -f "/tmp/$plugin.zip"; then
        ...
        echo "FEHLER: $plugin wurde nicht ersetzt. Nicht zum Download im WordPress.org-Plug-in-Repository verfügbar." >> $workingdirectory/wpsweeper-$datetime.log;
    fi
}
...
function sweep_plugin_withversion {
    ...
    if test -f "/tmp/$plugin.$pluginversion.zip"; then
        ...
        # Plugin not found - reporting
        echo "FEHLER: $plugin $pluginversion wurde nicht ersetzt. Nicht zum Download im WordPress.org-Plug-in-Repository verfügbar." >> $workingdirectory/wpsweeper-$datetime.log;
    fi
}
...
function sweep_wp_plugins {
    ...
    if [[ -d wp-content/plugins ]]
        ...
        echo "FEHLER: Ordner "wp-content/plugins" ist nicht vorhanden oder zugänglich." >> $workingdirectory/wpsweeper-$datetime.log;
    fi
    echo "" >> $workingdirectory/wpsweeper-$datetime.log;
    # Clear the screen and go back to the menu
    gotomenu
...
function sweep_wp_themes {
    clear
    echo "Ersetzen der Themes durch eine neue Version, die aus dem WordPress.org-Repository heruntergeladen wurde..."
    echo ""
    echo "######### WordPress Themes #########" >> $workingdirectory/wpsweeper-$datetime.log
    echo "" >> $workingdirectory/wpsweeper-$datetime.log
    echo "Unten findest du die Themes, die erfolgreich durch eine neue Version ersetzt wurden, die aus dem Theme-Repository von WordPress.org heruntergeladen wurde." >> $workingdirectory/wpsweeper-$datetime.log
    echo "Wenn ein Thema als FEHLER markiert ist, hat dies einen der folgenden Gründe:" >> $workingdirectory/wpsweeper-$datetime.log
    echo "- Das Theme ist ein Premium-Theme (das wir nicht automatisch für Sie ersetzen können)." >> $workingdirectory/wpsweeper-$datetime.log
    echo "- Das Theme wurde bereits aus dem Theme-Repository von WordPress.org entfernt." >> $workingdirectory/wpsweeper-$datetime.log
    echo "- Das Design ist ein benutzerdefiniertes Design, das nicht im Design-Repository von WordPress.org veröffentlicht ist." >> $workingdirectory/wpsweeper-$datetime.log
    echo "" >> $workingdirectory/wpsweeper-$datetime.log
            ...
            cd wp-content/themes/
                for theme in $(ls -d */ | cut -f1 -d'/'); do
                    ...
                    if test -f "/tmp/$theme.zip"; then 
                        ...
                        echo "FEHLER: $theme wurde nicht ersetzt. Nicht verfügbar im Theme-Repository von wordpress.org." >> $workingdirectory/wpsweeper-$datetime.log;
                    fi
                done
            cd $workingdirectory
        else
            echo "FEHLER: Ordner "wp-content/themes" ist nicht vorhanden oder zugänglich." >> $workingdirectory/wpsweeper-$datetime.log;
        fi
    echo "" >> $workingdirectory/wpsweeper-$datetime.log;
    # Clear the screen and go back to the menu
    gotomenu
...
function sweep_malware_scan {
    clear
    echo "Ausführen eines Malware-Scans."
    echo "Bitte warten... Dies kann eine Weile dauern."
    echo ""
    echo "######### Malware Scan #########" >> $workingdirectory/wpsweeper-$datetime.log
    echo "" >> $workingdirectory/wpsweeper-$datetime.log
    echo "Diese Infektionen oder verdächtigen Dateien wurden gefunden:" >> $workingdirectory/wpsweeper-$datetime.log
    echo "" >> $workingdirectory/wpsweeper-$datetime.log
    ...
    if [ -z $workingdirectory/infectionlist.log ]; then
        echo "Keine bekannten Infektionen oder bösartigen Dateien gefunden. Alles tiptop!" >> $workingdirectory/wpsweeper-$datetime.log
    else 
...
function wpsweeper_menu {
    clear
    display_logo
    echo "Treffen Sie Ihre Wahl, indem Sie die gewünschte Nummer eingeben.   "
    echo "                                                       "

select choice in \
    "WordPress-Core-Dateien durchsuchen" \
    "WordPress-Plugin-Dateien durchsuchen" \
    "WordPress Theme-Dateien durchsuchen" \
    "Malware-Scan" \
    "Beenden"
do
    case $choice in
        ...
        "Beenden")
            ...
            ;;
        *)
            echo "Bitte geben Sie die Nummer Ihrer Wahl ein.";
            ;;
    esac
done
