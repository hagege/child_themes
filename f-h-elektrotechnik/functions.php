 ...
 * für Child Theme: f-h-elektrotechnik (Parent: Ollie, Block Theme)
 * 
 * Hinweis: theme.json übernimmt globale Styles automatisch,
 * diese Funktion lädt zusätzlich das klassische style.css
 * für individuelles CSS, das nicht über theme.json abbildbar ist
 */
function fhe_enqueue_styles() {
    $theme = wp_get_theme();
    $parent = $theme->parent();

    // Optional: Nur laden, falls style.css tatsächlich Inhalt hat
	// evtl. noch entfernen:
    wp_enqueue_style(
        'ollie-style',
...
add_action('wp_enqueue_scripts', 'fhe_enqueue_styles');
